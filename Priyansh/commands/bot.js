...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
  ...
  var rand = tl[Math.floor(Math.random() * tl.length)]
    ...
    if ((event.body.toLowerCase() == "AR bot") || (event.body.toLowerCase() == "Sumu bot") || (event.body.toLowerCase() == "Chumtiya bot") || (event.body.toLowerCase() == "chumtiye bot")) {
     return api.sendMssage("Hmm... Tumi amar nam janle kivabe, tnx amar nam janar jonno😾", threadID);
   };

   if ((event.body.toLowerCase() == "🤮") || (event.body.toLowerCase() == "😝")) {
     return api.sendMessage("Koy mash coler😝", threadID);
   };

    if ((event.body.toLowerCase() == "🤗") || (event.body.toLowerCase() == "🤗")) {
     return api.sendMessage("Janu.. baby.. amay ektu joriye dhorba ☺️", threadID);
   };

   if ((event.body.toLowerCase() == "sumu") || (event.body.toLowerCase() == "Samanta Sumu")) {
     return api.sendMessage("sumure gece kita dorker amare kow, naile sumur inbox a nk dew", threadID);
   };
   ...
   if ((event.body.toLowerCase() == "bc") || (event.body.toLowerCase() == "bc")) {
     return api.sendMessage("BC Abar kita, tumr nam nakita🤔 ", threadID);
   };
   ...
   if ((event.body.toLowerCase() == "morning") || (event.body.toLowerCase() == "good morning")) {
     return api.sendMessage("Ꮆɵɵɗ morning na koiya, ek kaf caa coffee banai dew☕✨💫", threadID);
   };

   if ((event.body.toLowerCase() == "Rakib vai") || (event.body.toLowerCase() == "@Rakib Ali")) {
     return apiN Api.sendMessage("Rakib Boss ekhn amake niye kaj kortece, ektu pore sms back korbe❤️", threadID);
   };

   if ((event.body.toLowerCase() == "Jaydul") || (event.body.toLowerCase() == "Sultan") || (event.body.toLowerCase() == "priyansh") || (event.body.toLowerCase() == "prince")) {
     return api.sendMessage( "Eni holen Boss Rakib er bondhu, lucca bedira dure thak 😘",threadID);

       
   };

   if ((event.body.toLowerCase() == "owner") || (event.body.toLowerCase() == "Owner")) {
     return api.sendMessage("💝🥀𝐎𝐖𝐍𝐄𝐑:- ☞RAKIB BOSS☜ 💫\n🖤𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 Rakib🖤\n😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:- ☞ www.facebook.com/rakib.mobile.01601150650\n👋For Any Kind Of Help Contact On whatsapp  👉 01616092343😇", threadID);
   };

   if ((event.body.toLowerCase() == "tumar boss ke") || (event.body.toLowerCase() == "tumake k baniyece")) {
     return api.sendMessage("Rakib  ❤️ My Creator. He loves me & Edit Me Daily. tumaderke moja dewar jonno amake banaicen amar boss", threadID);
   };
